...
class ProfileManager(models.Manager):
    def create_profile(self, user, **kwargs):
        ...
        return ConsumerProfile.objects.create(
            user=user, group=group, phone_number=phone
        )
    ...
    def employee_update_by_owner(self, boss_profile, **kwargs):
        ...
        employee_profile: SupplierEmployeeProfile = SupplierEmployeeProfile.get(
            pk=employee_profile_pk
        )
...
class BaseProfile(models.Model):
    ...
    current_zipcode = models.ForeignKey(
        Zipcode, on_delete=models.SET_NULL, null=True, blank=True
    )
    latest_location = models.ForeignKey(
        Coordinate, on_delete=models.SET_NULL, null=True, blank=True
    )
...
class ConsumerProfile(BaseProfile):
    ...
    plan = models.ForeignKey(
        ConsumerPlan, on_delete=models.SET_NULL, null=True, blank=True
    )
...
class SupplierEmployeeProfile(BaseProfile):
    ...
    company = models.ForeignKey(
        SupplierCompany, on_delete=models.CASCADE, related_name="employees"
    )
...
class LibraryProduct(models.Model):
    product = models.ForeignKey(
        Product, null=True, on_delete=models.SET_NULL, related_name="project_products"
    )
    owner = models.ForeignKey(
        ConsumerProfile, on_delete=models.CASCADE, related_name="products"
    )
