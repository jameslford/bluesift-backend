...
def create_value_cleaner(request: Request, model_type):
    ...
    if not (field and new_value and old_value):
        return Response(
            "need field, new value and old value", status=status.HTTP_400_BAD_REQUEST
        )
    ValueCleaner.objects.create_and_apply_async(
        product_class=model_type, field=field, new_value=new_value, old_value=old_value
    )
...
def product_detail(request, pk):
    ...
    if request.method == "POST":
        ...
        response.update({"admin_fields": admin_fields})
        return Response(response, status=status.HTTP_200_OK)
