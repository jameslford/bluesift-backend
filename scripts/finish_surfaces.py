...
def assign_tiling_image():
    # desired_size = settings.DESIRED_IMAGE_SIZE - 2
    for product in FinishSurface.objects.all():
...
def __cluster_sizes(products: QuerySet):
    ...
    sizes = (
        products.filter(actual_size__isnull=False)
        .annotate(as_float=Cast("actual_size", output_field=FloatField()))
    ...
    medium_prods = products.filter(
        actual_size__isnull=False, actual_size__lte=medium, actual_size__gt=small
    )
    ...
    for tile in tiles:
        ...
        if fin:
            ...
            if len(split) > 1:
                ...
                ValueCleaner.create_or_update(
                    TileAndStone._meta.verbose_name_plural, "finish", new_fin, fin
                )
        look = tile.look
        if look:
            if "mosaic" in look:
                ...
                ValueCleaner.create_or_update(
                    TileAndStone._meta.verbose_name_plural, "look", new_look, look
                )
