    ...
    def __retrieve_cache(cls, res_dict, page, search_string):
        if search_string:
            ...
            products = (
                ...
                .filter(similarity__gte=0.1, pk__in=product_pks)
                .order_by("-similarity")[0:20]
            )
...
class Sorter:
    ...
    def calculate_response(self, static_queryset: QuerySet = None):
        ...
        pk_sets = [
            pks
            for pks in pk_sets
            if pks is not None and len(pks) < self.initial_product_count
        ]
        ...
        products = dynamic_queryset.intersection(static_queryset).values_list(
            "pk", flat=True
        )
