...
class BaseReturnValue:
    def __init__(
        self, expression: str, name: str, count: int = None, value=None, color=False
    ):
...
class BaseFacet(models.Model):
    ...
    attribute_list = pg_fields.ArrayField(
        models.CharField(max_length=60, null=True, blank=True), null=True, blank=True
    )
    content_type = models.ForeignKey(
        ContentType, on_delete=models.CASCADE, limit_choices_to=limit
    )
    static_charfield_values = pg_fields.ArrayField(
        models.CharField(max_length=80, null=True, blank=True), null=True, blank=True
    )
    ...
    def save(
        ...
        if self.attribute:
            ...
            if self.field_type != actual_type:
                raise Exception(
                    f"Attribute -{self.attribute}'s- field type {actual_type} != {self.field_type}"
                )
        if not self.name:
    ...
    def get_charfield_values(self):
        ...
        values = list(
            self.model.objects.values_list(self.attribute, flat=True).distinct()
        )
        if self.dynamic:
            return values
    ...
    def get_search_values(self):
        ...
        if self.attribute:
            ...
            return [
                {"tag": value, "express": f"{self.attribute}={value}"}
                for value in values
            ]
...
class QueryIndex(models.Model):
    ...
    retailer_location = models.ForeignKey(
        SupplierLocation, null=True, on_delete=models.CASCADE, related_name="qis"
    )
    products = models.ManyToManyField("Products.Product", related_name="query_indexes")
