...
class NumericFacet(BaseFacet):
    ...
    def __get_absolutes(self):
        ...
        if self.field_type in ["RangeField", "DecimalRangeField", "FloatRangeField"]:
            print(self.name)
            kwargs = {
                "min": Min(Lower(self.attribute)),
                "max": Max(Upper(self.attribute)),
            }
        else:
    ...
    def filter_self(self):
        ...
        if self.selected_max:
            self.enabled_values.append(
                BaseReturnValue(
                    f"{self.name}_max={self.selected_max}",
                    f"Max {self.name}",
                    None,
                    True,
                )
        ...
        if self.selected_min:
            self.enabled_values.append(
                BaseReturnValue(
                    f"{self.name}_min={self.selected_min}",
                    f"Min {self.name}",
                    None,
                    True,
                )
