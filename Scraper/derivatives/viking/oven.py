...
def run(group: ScraperGroup):
    ...
    for figure in figures:
        ...
        for finish in finishes:
            style = finish["style"]
            bgColor = (
                style.replace("background-color", "").strip()
            ...
            if bgColor and text:
                color = ApplianceColor.objects.get_or_create(
                    manufacturer=group.manufacturer, hex_value=bgColor, name=text
                )[0]
                colors.append(color)

        download_req = requests.get(cad_path)
        download_soup = BeautifulSoup(download_req.text, "lxml")
        downloads = download_soup.find(
            "div", {"class": "design-software-symbols container clearfix"}
        )
