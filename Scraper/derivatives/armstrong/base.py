...
def scrape(group: ScraperGroup, list_function, detail_function):
    model_type = group.get_model()
    ...
    for item in items:
        ...
        product.floors = True
        product = list_function(product, item)
        if not product:
            continue
        product = get_detail(product, detail_function)
