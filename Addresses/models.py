...
class AddressManager(models.Manager):
    def get_or_create_address(self, **kwargs):
        ...
        response = None
        add_obj = (
            self.model.objects.filter(gmaps_id=gmap_id).first() if gmap_id else None
        )
        if add_obj:
            return add_obj
...
class Address(models.Model):
    ...
    country = models.CharField(
        max_length=120, blank=True, null=True, default="United States of America"
    )
    state = models.CharField(max_length=120)
    postal_code = models.ForeignKey(
        Zipcode, on_delete=models.SET_DEFAULT, default=1, related_name="address"
    )
    ...
    def find_closest_others(self):
        return (
            Address.objects.filter(demo=True)
            .annotate(distance=Distance("coordinates__point", self.coordinates.point))
            .order_by("-distance")
        )
